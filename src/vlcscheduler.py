...
async def player_coro(player, rebuild_events_queue, extra_items_queue, ping_urls=None):
    ...
    while True:
        ...
        try:
            ...
            if item.path != current_item_path:
                logger.info('Playing %s for %i seconds.' %
                            (item.path, play_duration))
                ...
                for ping_url in ping_urls:
                    ...
                    try:
                        ...
                        logger.error(
                            'PING_URL failed={0} url={1}'.format(e, ping_url))
                    else:
                        if r.status_code != requests.codes.created:
                            logger.error('PING_URL status={0} url={1}'.format(
                                r.status_code, ping_url))
...
async def main_coro():
    ...
    logger.info('Rebuilds will run at: %s.' %
                ', '.join(sorted(rebuild_schedule)))

    # Setup coroutines
    tasks = [
        launcher.watch_exit(), schedule_coro(),
        player_coro(player, rebuild_events_queue,
                    periodic_items_queue, config.PING_URLS)
    ]
