...
class VLCLauncher:
    ...
    def check_connection(self, retries=0):
        for i in range(retries, -1, -1):
            try:
                ...
                if i > 0:
                    logger.warning(
                        'Connection attempt failed because of: %s. Retry in 3 seconds.' % str(
                            e)
                    )
                    time.sleep(3)
                    continue
    ...
    async def launch(self):
        try:
            ...
            logger.warning('Found existing VLC instance.')
            return

        logger.info('Launching VLC with HTTP server at %s.' %
                    self.config['path'])
